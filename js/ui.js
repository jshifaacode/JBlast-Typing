...
function buildMobileKeyboard() {
  ...
  container.innerHTML = layout.map((row, ri) =>
    `<div class="kb-row">${ri === 2 ? '<button class="key-btn key-back bb" data-key="BACK">←</button>' : ''}${row.split('').map(c => `<button class="key-btn bb" data-key="${c}">${c}</button>`).join('')}${ri === 2 ? '<button class="key-btn key-space bb" data-key=" ">SPC</button>' : ''}</div>`
  ).join('');

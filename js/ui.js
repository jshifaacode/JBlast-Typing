const UI = (() => {
  ...
  function buildAvatarGrid() {
    ...
    grid.innerHTML=avs.map((a,i)=>`<div class="avatar-item${i===0?' selected':''}" data-avatar="${a}">${a}</div>`).join('');
  ...
  function updateMenuDisplay(p) {
    ...
    const xp=p.xp||0, lvl=Math.floor(Math.sqrt(xp/50))+1;
    ...
    if(g('xpText')) g('xpText').textContent=(xp-curBase)+' / '+(nextBase-curBase)+' XP';
    const stats=p.stats||{};
  ...
  function buildStats(stats) {
    ...
    grid.innerHTML=items.map(i=>`<div class="stat-card"><span class="sv bb">${i.v}</span><span class="sl">${i.l}</span></div>`).join('');
  }
  function showResult({victory,wpm,accuracy,maxCombo,score,mpWinner,mpPlayers}) {
    ...
    if(g('resultTitle')){g('resultTitle').textContent=titleText;g('resultTitle').className='result-banner '+(victory?'win':'lose');}
    if(g('resultRank')){g('resultRank').textContent=rank;g('resultRank').className='result-rank rank-'+rank;}
    ...
    if(mpEl){
      if(mpWinner&&mpPlayers&&mpPlayers.length>0){
        ...
        const rows=sorted.map((pl,idx)=>{
          const isMeWinner=pl.id===mpWinner;
          const medal=idx===0?'🥇':idx===1?'🥈':'🥉';
          return`<div class="mp-result-row${isMeWinner?' mp-result-winner':''}">${medal} ${pl.avatar||'⚡'} ${pl.name}${pl.id===Multiplayer.getPlayerId()?' (YOU)':''}<span>${Math.max(0,Math.floor(pl.hp||0))} HP</span></div>`;
        }).join('');
        mpEl.innerHTML=`<div class="mp-result-box"><div class="mp-result-title bb">BATTLE RESULTS — RANKING</div>${rows}</div>`;
      } else { mpEl.innerHTML=''; }
    }
...
function buildMobileKeyboard() {
  const container=document.getElementById('mobileKeyboard'); if(!container) return;
  container.style.display='flex';
  const layout=['QWERTYUIOP','ASDFGHJKL','ZXCVBNM'];
  container.innerHTML=layout.map((row,ri)=>`<div class="kb-row">${ri===2?'<button class="key-btn key-back bb" data-key="BACK">←</button>':''}${row.split('').map(c=>`<button class="key-btn bb" data-key="${c}">${c}</button>`).join('')}${ri===2?'<button class="key-btn key-space bb" data-key=" ">SPC</button>':''}</div>`).join('');

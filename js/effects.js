var Effects = (function() {
  ...
  function spawnParticle(x, y, color) {
    ...
    for (var i = 0; i < 8; i++) {
      var a = ((Math.PI * 2) / 8) * i + Math.random() * 0.5,
          s = Math.random() * 3 + 1;
      particles.push({ x:x, y:y, vx:Math.cos(a)*s, vy:Math.sin(a)*s, life:1, decay:Math.random()*0.04+0.02, size:Math.random()*4+2, color:color });
    }
  }
  ...
  function spawnBurst(x, y, color, count) {
    ...
    for (var i = 0; i < count; i++) {
      ...
      particles.push({ x:x, y:y, vx:Math.cos(a)*s, vy:Math.sin(a)*s, life:1, decay:Math.random()*0.02+0.015, size:Math.random()*6+2, color:color });
    }
  }
  ...
  (function loop() {
    ...
    g.addColorStop(0.5, "rgba(191,0,255," + (0.007 + Math.cos(t * 0.8) * 0.004) + ")");
  ...
  function screenShake(intensity, dur) {
    ...
    if (!dur) dur = 300;
    var app = document.getElementById("app"), start = Date.now();
    (function shake() {
      var el = Date.now() - start;
      if (el >= dur) { app.style.transform = ""; return; }
      var d = 1 - el / dur;
      app.style.transform = "translate(" + (Math.random() - 0.5) * intensity * 2 * d + "px," + (Math.random() - 0.5) * intensity * d + "px)";
  ...
  function showDamageNumber(el, amount, color) {
    ...
    el.appendChild(num);
    setTimeout(function() { if (num.parentElement) num.remove(); }, 850);
    try {
  ...
  function typeEffect(el, skin) {
    ...
    try {
      var r = el.getBoundingClientRect();
      var cols = { fire:"#ff6600", lightning:"#ffe000", glitch:"#9d00ff", ice:"#00ccff", default:"#00f5ff" };
      particles.push({
        x: r.left + r.width / 2, y: r.top + r.height / 2,
        vx: (Math.random() - 0.5) * 2, vy: -Math.random() * 2 - 1,
        life: 0.8, decay: 0.06, size: Math.random() * 3 + 1,

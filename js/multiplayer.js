...
var Multiplayer = (function () {
  ...
  function _broadcastWinner(fp) {
    if (!isHost || !room || _gameEnded) return;
  ...
  function pushOpponentHp(oppId, hp) {
    if (!room || !pid || _gameEnded) return Promise.resolve();
    var safe = Math.max(0, Math.ceil(hp));
    if (players[oppId] !== undefined) players[oppId].hp = safe;
    return dbSet("rooms/" + room + "/players/" + oppId + "/hp", safe).then(
      function () {
        if (safe === 0 && !_gameEnded) {
          dbGet("rooms/" + room + "/players").then(function (fp) {
            if (!fp) return;
  ...
  function broadcastGameOver(winnerId) {
    if (!room || _gameEnded) return Promise.resolve();
    _gameEnded = true;
    var val = winnerId || "none";
    _lastGameOver = val;

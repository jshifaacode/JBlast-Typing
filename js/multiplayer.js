...
const Multiplayer = (() => {
  ...
  function startPoll(code) {
    _poll=setInterval(async()=>{
      const r=await get(`rooms/${code}`); if(!r) return;
      const snap=JSON.stringify(r.players||{});
      if(snap!==_lastSnap){_lastSnap=snap;players=r.players||{};emit('players_update',{players:Object.values(players)});_updateLobby();}
      if(!isHost&&r.status==='playing'&&_lastStatus!=='playing'){_lastStatus='playing';if(r.currentWord)emit('game_start',{word:r.currentWord,players:Object.values(players)});}
      if(r.status)_lastStatus=r.status;
    },1200);
  }

  function _entry(p) { return{id:p.id,name:p.name,avatar:p.avatar,hp:100,wpm:0,progress:0,ready:true,isBot:false,online:true}; }

  async function createRoom(p) {
    p.id=p.id||genId(); pid=p.id; isHost=true;
    const code=genCode(); room=code;
  ...
  async function joinRoom(code, p) {
    ...
    p.id=p.id||genId(); pid=p.id; isHost=false; room=code;
    await set(`rooms/${code}/players/${pid}`,_entry(p));
    const all=await get(`rooms/${code}/players`);
    players=all||{}; _listen(code); startPoll(code);
    window.addEventListener('beforeunload',()=>navigator.sendBeacon(`${FIREBASE_URL}/rooms/${code}/players/${pid}/online.json`,JSON.stringify(false)));
    return{roomCode:code,players:Object.values(players)};
  }
  ...
  function _listen(code) {
    listen(`rooms/${code}/players`,(data,path)=>{
      ...
      if(path==='/'||path===''){
        ...
        emit('hp_update',{playerId:p2,hp:players[p2]?.hp??100});
        _updateLobby();
      }
    });
    listen(`rooms/${code}/status`,(data)=>{
      if(data==='playing'&&!isHost){get(`rooms/${code}/currentWord`).then(w=>{if(w)emit('game_start',{word:w,players:Object.values(players)});});}
    });
  ...
  function _updateLobby() {
    const grid=document.getElementById('lobbyPlayers'); if(!grid) return;
    grid.innerHTML=Object.values(players).map(p=>`<div class="lpc ready"><div class="lpc-avatar">${p.avatar||'⚡'}</div><div class="lpc-name bb">${p.name}${p.id===pid?' (YOU)':''}</div><div class="lpc-status" style="color:var(--g)">READY ✓</div></div>`).join('');
    const st=document.getElementById('lobbyStatus'); if(st) st.textContent=Object.keys(players).length+' PILOT(S) IN LOBBY';
  }
  ...
  async function updatePlayerHp(p, hp) {
    if(!room) return;
    if(players[p]) players[p].hp=hp;
    emit('hp_update',{playerId:p,hp});
    await upd(`rooms/${room}/players/${p}`,{hp});
  }

  async function leaveRoom() {
    stopBots(); stopListeners();
    if(room&&pid){ await del(`rooms/${room}/players/${pid}`); if(isHost) await del(`rooms/${room}`); }
    room=null; players={}; isHost=false;
    emit('left_room',{});
  }

  function startBotSimulation(word) {
    _bots.forEach(clearInterval); _bots=[];
